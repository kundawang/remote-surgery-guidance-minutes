"""转写段与 diarization 说话人段的合并逻辑（纯函数，便于测试）。

规则：
1. 每个 diarization label 独立命名；同一角色对应多个 label 时按首次出现
   时间加序号（如 张博士1 / 张博士2），单 label 角色不加序号。同一 label
   在一次处理中只解析一次档案，因此始终对应同一个名字。
2. 一个转写段的 speaker 与 speaker_role 必须来自同一条被匹配到的
   diarization 段，不允许分开推断。
3. 时间匹配保持既有语义：优先取包含转写段中点的说话人段；没有则取中点
   距离最近的说话人段。多候选时按 (start_time, label) 确定性裁决，同一
   份输入重复跑、输入顺序打乱，结果完全一致。
4. 没有任何说话人段可匹配时保留原文，speaker 与 speaker_role 标注未知。
"""
...
def _segment_sort_key(seg: Dict[str, Any]) -> Tuple[float, str]:
    return (float(seg.get("start_time", 0.0)), str(seg.get("speaker")))
...
def build_speaker_profile_map(
    ...
    编号顺序只取决于 label 首次出现的时间与 label 本身，与输入列表顺序
    无关，避免同一批 diarization 段以不同顺序传入时编号翻转。
    """
    ...
    label_order = sorted(label_role, key=lambda lbl: (label_first_start[lbl], lbl))
...
def find_matching_segment(
    ...
    containing = [
        seg
        for seg in speaker_segments
        if float(seg["start_time"]) <= mid_time <= float(seg["end_time"])
    ]
    if containing:
        return min(containing, key=_segment_sort_key)

    def distance_key(seg: Dict[str, Any]) -> Tuple[float, float, str]:
        seg_mid = _midpoint(float(seg["start_time"]), float(seg["end_time"]))
        return (abs(mid_time - seg_mid),) + _segment_sort_key(seg)

    return min(speaker_segments, key=distance_key)
...
def attach_speaker_info(
    ...
    for seg in transcript_segments:
        ...
        matched = find_matching_segment(
            _midpoint(float(seg["start_time"]), float(seg["end_time"])),
            speaker_segments,
        )
        if matched is None:
