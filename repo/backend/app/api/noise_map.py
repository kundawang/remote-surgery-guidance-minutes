...
from ..core.database import get_db, NoiseDistrict, NoiseDevice, NoiseRecording, NoiseReportPoint

router = APIRouter()

# 平均值统一保留的小数位数，接口内所有 avg_noise_level 都走这个精度
AVG_NOISE_DECIMALS = 2


def _valid_noise_levels(recordings):
    # 只取真正有效的噪声值：None 视为无数据被剔除；0.0 是合法低噪声值，必须保留。
    # 注意不能用 `if rec.noise_level` 判断，否则 0 会被误当成"没有值"丢掉。
    return [rec.noise_level for rec in recordings if rec.noise_level is not None]


def _average(levels):
    # 分母只用参与计算的有效数据量，绝不用总记录数；
    # 没有有效值时返回 None（null），明确表示"无数据"，而不是伪造一个均值。
    if not levels:
        return None
    return round(sum(levels) / len(levels), AVG_NOISE_DECIMALS)


def _attribute_district_id(point, devices):
    # 举报点归属规则（对同一份数据结果确定）：
    # 按经纬度欧氏距离把举报点归到"最近设备"所在的区域；
    # 距离相同（含同区域多设备）时取设备 id 较小者，保证可重复。
...
def get_district_stats(
    ...
    recording_query = db.query(NoiseRecording).filter(NoiseRecording.device_id.in_(device_ids or [-1]))
    ...
    for district_id, district_recordings in recordings_by_district.items():
        levels = _valid_noise_levels(district_recordings)
        stats = stats_by_district[district_id]
        # recordings_count 就是参与平均/最大值计算的有效数据量，与分母严格一致
    ...
    for point in report_points:
        district_id = _attribute_district_id(point, devices)
