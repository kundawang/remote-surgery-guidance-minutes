...
class SpeakerDiarization:
    ...
    def get_speaker_statistics(self, segments: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for seg in segments:
            ...
            end_time = float(seg.get("end_time", start_time + seg.get("duration", 0.0)))
        ...
        for speaker, intervals in speaker_intervals.items():
            ...
            speaker_stats[speaker] = {
                "total_duration": total_duration(intervals),
    ...
    def get_total_duration(self, segments: List[Dict[str, Any]],
        ...
        """统计所有说话区间的并集总时长；传入音频长度时以其为上限。"""
        intervals = [
            (
                float(seg.get("start_time", 0.0)),
                float(seg.get(
                    "end_time",
                    seg.get("start_time", 0.0) + seg.get("duration", 0.0)
                ))
            )
            for seg in segments
        ]
    ...
    def resolve_session_speakers(self, db, session_db_id: int,
        ...
        existing = db.query(Speaker).filter(
            Speaker.session_id == session_db_id
        ).all()
        by_label = {speaker.label: speaker for speaker in existing}
        next_index = max(
            (speaker.speaker_index for speaker in existing), default=0
        ) + 1
