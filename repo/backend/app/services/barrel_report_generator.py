...
class BarrelReportGenerator:
    ...
    def _build_title(self, request: BarrelTastingRequest) -> str:
        wine_name = f"{request.wine_name}" if request.wine_name else ""
...
请用中文回答，保持专业性和准确性。"""
        ...
        parcel_lines = []
        for parcel in self._sorted_parcels(request.parcels):
            metrics = ", ".join(
...
"""
    ...
    def _generate_fallback(
        ...
        for parcel in self._sorted_parcels(request.parcels):
            ...
            metrics_str = f"，理化指标: {metrics}" if metrics else ""
            tasting_parts.append(
                f"地块 {parcel.name}（{parcel.variety}，{parcel.volume_liters}L{metrics_str}）"
            )
