...
class SegmentBoundaryTests(unittest.TestCase):
    def test_boundary_order_counts_only_once(self):
        # 复现场景：[0,100) 与 [100,200)，t=100 只能归到第二段
        ...
        self.assertEqual(report["segments"][1]["orders"][0]["seconds"], 100.0)

    def test_segment_totals_equal_detail_totals(self):
        # 多个边界点 0/100/200，加上落在所有段之外的 300
        ...
        unassigned_revenue = sum(float(o.get("amount", 0.0)) for o in report["unassigned_orders"])
        self.assertAlmostEqual(segment_revenue + unassigned_revenue, float(sum(range(1, 9))))

        # t=300 是所有段的右端点，左闭右开下不归任何段，也绝不重复计数
        self.assertEqual([o["seconds"] for o in report["unassigned_orders"]], [300.0])
    ...
    def test_empty_inputs(self):
        report = assign_orders_to_segments([], [{"start": 0, "end": 100}])
        self.assertEqual(report["segments"][0]["order_count"], 0)
        report = assign_orders_to_segments(make_orders([1, 2]), [])
        self.assertEqual(len(report["unassigned_orders"]), 2)


class BurstDetectionTests(unittest.TestCase):
    def test_peak_does_not_pollute_its_own_baseline(self):
        # 复现场景：[20]*10 + [400]，基线只看当前点之前的数据
        alerts = detect_bursts([20] * 10 + [400])
    ...
    def test_threshold_semantics_unchanged(self):
        ...
        alerts = detect_bursts([4, 8, 4, 8, 10], z_threshold=2.0, window=5)
        boundary = [alert for alert in alerts if alert["order_count"] == 10]
        self.assertEqual(len(boundary), 1)
        self.assertAlmostEqual(boundary[0]["z_score"], 2.0)
        # 当前点 9 -> z=1.5，不报
        below = detect_bursts([4, 8, 4, 8, 9], z_threshold=2.0, window=5)
        self.assertNotIn(9, [alert["order_count"] for alert in below])

    def test_dict_points_keep_fields(self):
        points = [
            {"timestamp": f"00:00:{i:02d}0", "seconds": i * 10, "order_count": 20}
            for i in range(5)
